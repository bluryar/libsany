  ...
  setup(props, { slots }) {
    ...
    return () => (
      <div class="fake-dialog-wrapper">
        {visible.value ? (
          <div class="fake-dialog-content">{slots?.default?.()}</div>
        ) : (
          <div class="fake-dialog-empty"></div>
        )}

        <div class="fake-dialog-close-btn" onClick={onClick}>
          关闭按钮
        </div>
      </div>
    );
...
const { visible, foo, obj } = useVModels(props, undefined, { passive: !!1, deep: !!1 });
...
<template>
  <FakeDialog v-model:visible="visible">
    <div class="use-dialog" @click="onClick">foo: {{ foo }} obj.test: {{ obj.test }}</div>
